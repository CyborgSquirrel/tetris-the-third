...
impl Command {
	pub fn execute(
		...
		let event = NetworkEvent::UnitCommand {command:self.clone(),unit_id};
		network_state.broadcast_event(&event);
		match self {
			Command::MoveLeft =>
					...
					if add_mino {
						...
						if !can_add {
							...
							if clearable_lines > 0 {
								...
								if let Mode::Marathon {level,lines_before_next_level,..} = &mut base.mode {
								...
								}else if let Mode::Versus {target_unit_id,lines_received,..} = &mut base.mode {
									if let Kind::Local {..} = unit.kind {
										...
										while let Some(lines) = lines_received.pop_front() {
											// TODO: add rng for this
											queue.push_back((unit_id, Command::AddLines(lines, 1)));
										}
									}
								}
							}
